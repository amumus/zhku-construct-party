...
public class JedisUtilService {
	...
	public Hash getHASH() {
		if (HASH == null) {
			...
			synchronized (JedisUtilService.class) {
				if (HASH == null) {
					HASH = new Hash();
				}
			}
	...
	public void returnJedis(Jedis jedis,int ym) {
		jedis.close();
	}
	...
	public class Strings {
		...
		public String get(String key, final int pool, final int ym) {
			// ShardedJedis sjedis = getShardedJedis();
			Jedis sjedis = getJedis(ym);
			sjedis.select(pool);
			String value = sjedis.get(key);
			returnJedis(sjedis,ym);
			return value;
		}
		/**
		 * 根据key获取记录
		 ...
		 * */
		public List<String> mget(final String[] key, final int pool, final int ym) {
			Jedis jedis = getJedis(ym);
			jedis.select(pool);
			List<String> str = jedis.mget(key);
			returnJedis(jedis,ym);
			return str;
		}
		 ...
		 * @return String 状态码
		 * */
		public String mset(final String[] split, final int pool, final int ym) {
			Jedis jedis = getJedis(ym);
			jedis.select(pool);
			String str = jedis.mset(split);
			returnJedis(jedis,ym);
			return str;
		}
		/**
		 ...
		 * @return String 状态码
		 * */
		public void del(final String key, final int pool, final int ym) {
			Jedis jedis = getJedis(ym);
			jedis.select(pool);
			jedis.del(key);
			returnJedis(jedis,ym);
